...
#   )
# saveRDS(bfm_binom, "data/bayes_glmm_normprior_interact.rds")
# 
# # without interaction
# bfm_binom_nointeract <- update(
#   bfm_binom, formula = ~ . -movement : word_type  # Same but without the interaction term
#   )  
# saveRDS(bfm_binom_nointeract, "data/bayes_glmm_normprior_nointeract.rds")
```


```{r}
# Once fit and saved, we can load models from disk:
bfm_binom <- readRDS("data/bayes_glmm_normprior_interact.rds")
bfm_binom_nointeract <- readRDS("data/bayes_glmm_normprior_nointeract.rds")
```
...
Or with a prettier and somewhat simplified layout (note that effects are
converted from log-odds to simple odds!):
...
(in log-odds, see the table for CI converted to odds).
...
# saveRDS(BF_bfm, "data/BF_bfm.rds")
```

```{r}
# Once run, load from disk:
BF_bfm <- readRDS("data/BF_bfm.rds")
BF_bfm
```


This BF consists "strong" evidence (BF > 10) in favour of the alternative hypothesis
...
`brms` also lets us plot the posterior distribution of the model estimates. We 
observe that the posterior distribution for the interfefence effect (4th panel
in left column) is clearly distinct from zero:
...
`r round(fixef(bfm_binom)[4, 1], 2)` log-odds is small. For instance, 
...
# write_csv(bootstr_res, "data/bootstrap_results.csv")
```
...
```{r, message=FALSE}
# Read from disk
bootstr_res <- read_csv("data/bootstrap_results.csv")
# str(bootstr_res)
head(bootstr_res)
```
